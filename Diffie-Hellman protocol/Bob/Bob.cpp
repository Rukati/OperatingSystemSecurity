...
string ReadMsg()
{
	// ѕолучаем строку от сервера
	char* buffer = NULL;
	int numBytesReceived = 0;
	int bufferSize = 1024;

	do {
		buffer = (char*)realloc(buffer, bufferSize); // ¬ыдел€ем динамическую пам€ть дл€ буфера
		numBytesReceived += recv(Conn, buffer + numBytesReceived, bufferSize - numBytesReceived, 0);
		if (numBytesReceived == bufferSize) {
			bufferSize *= 2; // ”величиваем размер буфера в два раза, если прин€тые данные заполнили весь буфер
		}
...
int main(int argc, char* argv[])
{
	...
	WORD DLLVersion = MAKEWORD(2, 1);//верси€ библиотеки
	if (WSAStartup(DLLVersion, &wsaData) != 0) // загрузка библиотеки
	{
		cout << "Error WSAData" << endl;
		exit(1);
	}
	SOCKADDR_IN addr; // структура преднозначена€ дл€ хранени€ адреса
