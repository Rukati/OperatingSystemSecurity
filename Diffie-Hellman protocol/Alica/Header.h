...
public:
	...
	BIGNUM* Partical(BIGNUM* public_key2) {
		...
		BN_mod_exp(partical_key, public_key, secret_key, public_key2, BN_CTX_new());
		std::cout << "\nPartical key >>>> " << BN_bn2dec(partical_key) << std::endl;
		return partical_key;
	}

	BIGNUM* Full(BIGNUM* partical2, BIGNUM* public_key2) {
		// Вычисление полного ключа
		BIGNUM* full_key = BN_new();
		BN_mod_exp(full_key, partical2, secret_key, public_key2, BN_CTX_new());
		std::cout << "--------------------------------\n";
		std::cout << "part: " << BN_bn2dec(partical2) << " \nsecret: " << BN_bn2dec(secret_key) << "\nPubl: " << BN_bn2dec(public_key2);
...
public:
	...
	std::string encrypt_message(const std::string& message, int key) {
		...
		std::cout << "\tEncryptedMsg >>>> " << encrypted_message;
	...
	std::string decrypt_message(const std::string& encrypted_message, int key) {
		...
		std::cout << "\tDecryptedMsg >>>> " << decrypted_message;
